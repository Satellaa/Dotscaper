...
class TCGCornerScraper:
	...
	def scrape(self) -> list[CardPrice]:
		...
		while True:
			...
			self.tcg_corner_endpoint = f"https://tcg-corner.com/collections/yu-gi-oh-single-card-asia-english?&page={page}"
	...
	def parse_card_price(self, item) -> Optional[CardPrice]:
		try:
			...
			raw_price = item.css_first("span.price-item--regular[data-product-price]").text()
			...
			set_number = info[0]
			rarity = info[-1]
			price = int(raw_price.split(" ")[0].replace("¥", "").replace(",", ""))
			status = item.css_first("li.product-card__label product-card__label--sold-out") and "For Sale" or "Sold Out"
	...
	def id_from_info(self, info: str) -> int:
		return int(str(int.from_bytes(info.upper().strip().encode(), byteorder='big'))[0:6])