...
class Executor:
	...
	async def update_prices(self, update_bigweb: bool=True, update_tcg_corner: bool=False):
		async with open_nursery() as nursery:
			if update_bigweb:
				nursery.start_soon(run_sync, self.task, self.bigweb_scraper, self.bigweb_updater)
			if update_tcg_corner:
				nursery.start_soon(run_sync, self.task, self.tcg_corner_scraper, self.tcg_corner_updater)
