...
class CardManager:
	...
	def setup_executor(self):
		print("Setup MongoDB...")
		print("Tips: To avoid having to enter the URI, DB_NAME, and COLL_NAME again, you can add them to the .env file.")
		
		uri = os.getenv("URI") or input("Enter your MongoDB URI: ")
		db_name = os.getenv("DB_NAME") or input("Enter your MongoDB Database name: ")
		coll_name = os.getenv("COLL_NAME") or input("Enter your MongoDB Collection name: ")
		
		self.executor = Executor(uri, db_name, coll_name)
	
	def handle_choice(self, update_info=False, update_prices=False):
		if update_info:
			source = self.get_user_choice("Card information", ["YAML Yugi", "Yugipedia"])
			self.update_card_info(source)
		elif update_prices:
			source = self.get_user_choice("Card prices", ["Bigweb", "TCG Corner"])
			self.update_card_prices(source)
	
	def get_user_choice(self, update_type, sources):
		print(f"Select the update source {update_type}:")
